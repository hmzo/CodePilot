...
**Claude Code 用デスクトップ GUI** -- Claude Agent SDK の上に完全なデスクトップ体験を提供し、認証情報とモデルは `~/.claude` から直接読み取ります。MCP とスキルで拡張、スマートフォンからリモート制御、アシスタントがあなたのワークフローを学習。
...
1. [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code/overview) をインストール: `npm install -g @anthropic-ai/claude-code`
2. `claude` を一度実行してログインを完了（または `ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL` を `~/.claude/settings.json` に手動で記述）
3. 上の[ダウンロード](#ダウンロード)セクションからプラットフォームに合った CodePilot インストーラーをダウンロード
4. CodePilot を起動 — 認証情報は `~/.claude` から自動的に読み取られます
5. 入力欄でモデルを選択して会話を開始
...
1. **`~/.claude` が設定済みであることを確認** — Claude Code CLI（`npm install -g @anthropic-ai/claude-code`）をインストールし、`claude` を一度実行してログインを完了するか、`ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL` を `~/.claude/settings.json` に手動で記述。
...
必要です。CodePilot は `claude` の上に乗った GUI で、認証情報 / Base URL / デフォルトモデルは `~/.claude/settings.json`（または `claude` ログインセッション）に依存します。CodePilot を起動する前に `npm install -g @anthropic-ai/claude-code` を実行し、`claude` を一度実行してログインを完了するか、`ANTHROPIC_AUTH_TOKEN` / `ANTHROPIC_BASE_URL` を設定ファイルに手動で記述してください。
</details>
